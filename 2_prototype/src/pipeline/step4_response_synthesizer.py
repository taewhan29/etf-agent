...
def synthesize_response(exec_info: dict) -> dict:
    ...
    if api_key and ground_truth_context:
        prompt = (
            f"당신은 한국투자신탁운용 ACE ETF 공식 안내 AI 파트너입니다.\n"
            f"아래 [도구 실행 데이터]에 수록된 수치와 팩트 정보만을 100% 근거로 하여 사용자의 질문에 친절하고 부드러운 대화체 서술형 문장으로 답변하십시오.\n"
            f"단순히 키워드나 숫자를 나열하지 말고, 핵심 정보(총보수율, AUM, 분배금 주기, 비용 계산 결과 등)를 자연스러운 안내체('~입니다', '~를 안내해 드립니다')로 정돈하십시오.\n"
            f"데이터에 없는 숫자는 절대 새로 지어내지 마시고, 아스테리스크(*) 기호와 이모티콘은 절대 사용하지 마십시오.\n\n"
            f"[사용자 질문]: {query}\n\n"
            f"[도구 실행 데이터]:\n{ground_truth_context}\n\n"
            f"[친절한 대화체 답변]:"
        )
